...
class NeuralNetwork(object):
    ...
    def train_many(self, xs, ys):
        self._count+=1
        self._session.run(self._trainer, feed_dict={self._x: xs, self._y_truth: ys})
        if self._count%30==0:
            pred = self._session.run(self._loss, feed_dict={self._x: xs, self._y_truth: ys}) 
            print("------------------------------------------------------")
            print("loss:",pred)
            filename = "loss.txt"
            with open(filename,'a') as f: 
                f.write(str(self._count)+" loss: "+str(pred)+"\n")
            print("------------------------------------------------------")

    def save(self, save_path="./zheship.ckpt"):
        save_path = self._saver.save(self._session, save_path)
        print("model saved to file: {}".format(save_path))

    def load(self, load_path="./zheship.ckpt"):
...
class Controller(object):
    ...
    def __init__(self, learning_rate=0.01):
        self._nn = NeuralNetwork(67, 3, [11, 7], learning_rate)
